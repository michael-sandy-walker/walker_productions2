...
public class JNACrypt32Utils {
   ...
   */
  public static byte[] protect(byte[] data) throws Exception {	  
    ...
    if (!rc) {
      ...
      throw new Exception("CryptProtectData failed: " + drc.intValue());
    }
    else {
   ...
   * @return the the protected form the data
   */
  public static byte[] unprotect(byte[] data) throws Exception {
    Memory input = new Memory(data.length);
    input.write(0, data, 0, data.length);
    Crypt32.DATA_BLOB in = new Crypt32.DATA_BLOB();
    in.cbData = new W32API.DWORD(data.length);
